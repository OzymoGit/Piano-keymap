...
class MidiToQwertyTab(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.moduleSelector = ctk.CTkOptionMenu(
            self.midiFrame, width=130, command=midiToQWERTYFunctions.onModuleSelect, values=["pynput", "keyboard", "win32msg"], 
        ...
        self.__class__.moduleSelector = self.moduleSelector
        ...
        # --- Target Window selector (win32msg mode) ---
        if osName == "Windows":
            self.targetWindowLabel = ctk.CTkLabel(
                self.midiFrame, text="Target Window", fg_color="transparent",
                font=customTheme.globalFont14,
                text_color=customTheme.activeThemeData["Theme"]["Settings"]["TextColor"]
            )
            self.targetWindowLabel.grid(row=6, column=0, padx=(0, 270), pady=(8, 0), sticky="es")
            self.__class__.targetWindowLabel = self.targetWindowLabel

            self.targetWindowDropdown = ctk.CTkOptionMenu(
                self.midiFrame, width=330, values=["— select a window —"],
                command=midiToQWERTYFunctions.onTargetWindowSelect,
                font=customTheme.globalFont14, dropdown_font=customTheme.globalFont14,
                fg_color=customTheme.activeThemeData["Theme"]["Settings"]["OptionBackColor"],
                dropdown_fg_color=customTheme.activeThemeData["Theme"]["Settings"]["OptionDropdownBackground"],
                button_color=customTheme.activeThemeData["Theme"]["Settings"]["OptionDropdownButtonColor"],
            ...
            self.targetWindowDropdown.grid(row=7, column=0, padx=(0, 40), pady=(0, 4), sticky="e")
            self.__class__.targetWindowDropdown = self.targetWindowDropdown
            ToolTip.CreateToolTip(self.targetWindowDropdown, text='Target window to send keystrokes to (win32msg mode)')

            self.refreshTargetWindows = ctk.CTkButton(
                self.midiFrame, image=customTheme.resetImageCTk, text="", width=30,
                command=midiToQWERTYFunctions.refreshTargetWindows,
                font=customTheme.globalFont14,
                fg_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["ButtonColor"],
                hover_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["ButtonHoverColor"],
                text_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["TextColor"],
            ...
            self.refreshTargetWindows.grid(row=7, column=0, padx=(330, 0), pady=(0, 4))
            self.__class__.refreshTargetWindows = self.refreshTargetWindows
            ToolTip.CreateToolTip(self.refreshTargetWindows, text='Refresh window list')

            # Enable/disable based on current module
            midiToQWERTYFunctions.updateTargetWindowWidgets()
