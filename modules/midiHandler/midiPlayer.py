...
class MidiPlayerTab(ctk.CTkFrame):
    def __init__(self, master):
        ...
        # TIMELINE SEEK BAR  (replaces static timelineIndicator label)
        self.seekBarFrame = ctk.CTkFrame(
            self.midiFrame,
            fg_color="transparent"
        )
        self.seekBarFrame.grid(row=11, column=0, padx=20, pady=(8, 0), sticky="ew")
        self.seekBarFrame.grid_columnconfigure(1, weight=1)

        self.timelineCurrentLabel = ctk.CTkLabel(
            self.seekBarFrame, text="0:00:00",
            font=customTheme.globalFont12,
            text_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["TextColor"],
            width=56, anchor="e"
        )
        self.timelineCurrentLabel.grid(row=0, column=0, padx=(0, 6))
        self.__class__.timelineCurrentLabel = self.timelineCurrentLabel

        self.seekSlider = ctk.CTkSlider(
            self.seekBarFrame,
            from_=0, to=1,
            fg_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["SpeedSliderBackColor"],
            progress_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["PlayingColor"],
            button_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["SpeedSliderCircleColor"],
            button_hover_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["PlayColor"],
            command=None  # live drag does nothing; seek fires on release
        )
        self.seekSlider.set(0)
        self.seekSlider.grid(row=0, column=1, sticky="ew", padx=4)
        self.__class__.seekSlider = self.seekSlider
        ToolTip.CreateToolTip(self.seekSlider, text='Drag or click to seek to any position')

        self.timelineTotalLabel = ctk.CTkLabel(
            self.seekBarFrame, text="0:00:00",
            font=customTheme.globalFont12,
            text_color=customTheme.activeThemeData["Theme"]["MidiPlayer"]["TextColor"],
            width=56, anchor="w"
        )
        self.timelineTotalLabel.grid(row=0, column=2, padx=(6, 0))
        self.__class__.timelineTotalLabel = self.timelineTotalLabel

        # Hidden compatibility shim: other code still sets .timelineIndicator.configure(text=...)
        # We intercept that by keeping a label that parses the text and updates the two labels.
        class _TimelineShim:
            """Forwards configure(text='H:MM:SS / H:MM:SS') to the two visible labels."""
            def __init__(self_inner, current_label, total_label):
                self_inner._cur = current_label
                self_inner._tot = total_label

            def configure(self_inner, **kwargs):
                text = kwargs.get("text", "")
                parts = str(text).split(" / ")
                if len(parts) == 2:
                    self_inner._cur.configure(text=parts[0])
                    self_inner._tot.configure(text=parts[1])

            def after(self_inner, delay, fn):
                self_inner._cur.after(delay, fn)

        self.timelineIndicator = _TimelineShim(self.timelineCurrentLabel, self.timelineTotalLabel)
        self.__class__.timelineIndicator = self.timelineIndicator

        # Bind seek on mouse release
        def _on_seek_release(event):
            midiPlayerFunctions.seekPlayback(self.seekSlider.get())
        self.seekSlider.bind("<ButtonRelease-1>", _on_seek_release)
