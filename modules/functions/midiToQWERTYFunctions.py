...
def onModuleSelect(value):
    try:
        ...
        configuration.configData.save()
        updateTargetWindowWidgets()
    except Exception as e:
        logger.exception(f"onModuleSelect error: {e}")

def onTargetWindowSelect(value):
    try:
        if value and value != "\u2014 select a window \u2014":
            configuration.configData["midiToQwerty"]["targetWindow"] = value
            configuration.configData.save()
    except Exception as e:
        logger.exception(f"onTargetWindowSelect error: {e}")

def refreshTargetWindows():
    try:
        from ui.midiToQWERTY import MidiToQwertyTab
        if osName != 'Windows':
            return
        from modules.midiHandler import midiToQWERTYWindows as _handler
        windows = _handler.listOpenWindows()
        if not windows:
            windows = ["\u2014 no windows found \u2014"]
        MidiToQwertyTab.targetWindowDropdown.configure(values=windows)
        saved = configuration.configData.get("midiToQwerty", {}).get("targetWindow", "")
        if saved and saved in windows:
            MidiToQwertyTab.targetWindowDropdown.set(saved)
        else:
            MidiToQwertyTab.targetWindowDropdown.set(windows[0])
    except Exception as e:
        logger.exception(f"refreshTargetWindows error: {e}")

def updateTargetWindowWidgets():
    try:
        from ui.midiToQWERTY import MidiToQwertyTab
        if osName != 'Windows':
            return
        module = configuration.configData.get("midiToQwerty", {}).get("inputModule", "pynput")
        isWin32 = module == "win32msg"
        state = "normal" if isWin32 else "disabled"
        MidiToQwertyTab.targetWindowDropdown.configure(state=state)
        MidiToQwertyTab.refreshTargetWindows.configure(state=state)
        if isWin32:
            refreshTargetWindows()
    except Exception as e:
        logger.exception(f"updateTargetWindowWidgets error: {e}")
