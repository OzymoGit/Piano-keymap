...
def midiModuleSelect(value):
    try:
        ...
        midiUpdateTargetWindowWidgets()
    except Exception as e:
        logger.exception("Error in midiModuleSelect")

def midiOnTargetWindowSelect(value):
    try:
        if value and value != "\u2014 select a window \u2014":
            configuration.configData["midiPlayer"]["targetWindow"] = value
            configuration.configData.save()
    except Exception as e:
        logger.exception("Error in midiOnTargetWindowSelect")

def midiRefreshTargetWindows():
    try:
        import platform
        if platform.system() != "Windows":
            return
        from modules.midiHandler import midiWindows as _handler
        windows = _handler.listOpenWindowsMidi()
        if not windows:
            windows = ["\u2014 no windows found \u2014"]
        SettingsTab.midiTargetWindowDropdown.configure(values=windows)
        saved = configuration.configData.get("midiPlayer", {}).get("targetWindow", "")
        if saved and saved in windows:
            SettingsTab.midiTargetWindowDropdown.set(saved)
        else:
            SettingsTab.midiTargetWindowDropdown.set(windows[0])
    except Exception as e:
        logger.exception("Error in midiRefreshTargetWindows")

def midiUpdateTargetWindowWidgets():
    try:
        import platform
        if platform.system() != "Windows":
            return
        module = configuration.configData.get("midiPlayer", {}).get("inputModule", "pynput")
        isWin32 = module == "win32msg"
        state = "normal" if isWin32 else "disabled"
        SettingsTab.midiTargetWindowDropdown.configure(state=state)
        SettingsTab.midiRefreshTargetWindowsBtn.configure(state=state)
        if isWin32:
            midiRefreshTargetWindows()
    except Exception as e:
        logger.exception("Error in midiUpdateTargetWindowWidgets")
...
def changeMidiMinSeekDelay(value):
    try:
        val = float(value)
        configuration.configData["midiPlayer"]["minSeekDelay"] = val
        configuration.configData.save()
        logger.debug("MidiMinSeekDelay changed to %s", val)
    except Exception as e:
        logger.exception("Error in changeMidiMinSeekDelay")

def changeMidiMaxSeekDelay(value):
    try:
        val = float(value)
        configuration.configData["midiPlayer"]["maxSeekDelay"] = val
        configuration.configData.save()
        logger.debug("MidiMaxSeekDelay changed to %s", val)
    except Exception as e:
        logger.exception("Error in changeMidiMaxSeekDelay")
...
def initControlSets():
    ...
    try:
        ...
        controlSets = {
            ...
            "midiMinSeekDelay": {
                "slider": SettingsTab.midiMinSeekDelaySlider,
                "entry": SettingsTab.midiMinSeekDelayEntry,
                "default": 2.0,
                "min": 0.0,
                "max": 10.0,
                "type": float,
                "onChange": lambda v: changeMidiMinSeekDelay(v)
            },
            "midiMaxSeekDelay": {
                "slider": SettingsTab.midiMaxSeekDelaySlider,
                "entry": SettingsTab.midiMaxSeekDelayEntry,
                "default": 3.0,
                "min": 0.0,
                "max": 10.0,
                "type": float,
                "onChange": lambda v: changeMidiMaxSeekDelay(v)
            },
            "midiSpeedFail": {
